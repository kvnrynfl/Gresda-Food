    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-8">
            ...
            <div class="mb-8 text-center text-gray-500">
                <i class="fas fa-lock-open text-5xl mb-4 text-primary"></i>
                <p>Masukkan kata sandi baru Anda di bawah ini.</p>
            </div>

            <form action="<?= BASEURL ?>/auth/processReset" method="POST" class="space-y-6">
                <?= CSRF::getTokenField() ?>
                <input type="hidden" name="token" value="<?= htmlspecialchars($token ?? '') ?>">

                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Email</label>
                    <input type="email" value="<?= htmlspecialchars($email ?? '') ?>" disabled class="w-full px-4 py-3 bg-gray-100 border border-gray-300 rounded-xl text-gray-500 font-medium cursor-not-allowed">
                </div>
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-2">Minimal 8 karakter, mengandung huruf besar dan angka.</p>
                </div>

                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Konfirmasi Kata Sandi</label>
                    <input type="password" name="confirm_password" required placeholder="Ulangi kata sandi baru" class="w-full px-4 py-3 bg-white border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary focus:border-cyan-700 transition">
                </div>
                ...
                <button type="submit" class="w-full bg-primary hover:bg-cyan-800 text-white font-bold py-3 px-4 rounded-xl shadow-lg shadow-cyan-600/30 transition transform hover:-translate-y-0.5 active:translate-y-0">
                    <i class="fas fa-key mr-2"></i> Reset Kata Sandi
                </button>
            </form>
        </div>
    </div>
