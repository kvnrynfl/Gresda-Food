...
<div class="mb-8 flex items-center justify-between">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-[26px] font-black text-slate-900 tracking-tight leading-none mb-1">Modifikasi Metode Pembayaran</h2>
            <p class="text-[13px] font-medium text-slate-500">Sesuaikan properti kanal pembayaran terpilih</p>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-[24px] shadow-[0_2px_20px_-8px_rgba(0,0,0,0.1)] border border-slate-200/60 overflow-hidden w-full relative mb-8">
    ...
    <div class="p-8 sm:p-10">
        <form action="<?= BASEURL ?>/admin/editPaymentMethod/<?= $method['id'] ?>" method="POST" enctype="multipart/form-data" class="space-y-8">
            <?= CSRF::getTokenField() ?>
                ...
                <div>
                    ...
                    <div class="flex gap-4 p-1.5 bg-slate-100 rounded-xl border border-slate-200/60 p-1">
                        <label class="relative flex-1 flex items-center justify-center cursor-pointer">
                            <input type="radio" name="type" value="bank_transfer" <?= $method['type'] === 'bank_transfer' ? 'checked' : '' ?> class="peer sr-only">
                        ...
                        <label class="relative flex-1 flex items-center justify-center cursor-pointer">
                            <input type="radio" name="type" value="e_wallet" <?= $method['type'] === 'e_wallet' ? 'checked' : '' ?> class="peer sr-only">
                ...
                <div>
                    <?php 
                        $props = [
                            ...
                            'value' => htmlspecialchars($method['account_name']),
                            'icon' => 'fas fa-user',
                            'required' => true
                        ];
                        include __DIR__ . '/../components/ui/input.php';
            ...
            <div>
                ...
                <textarea name="instructions" rows="4" class="w-full px-5 py-3.5 bg-slate-50/50 border border-slate-200/80 rounded-xl text-slate-900 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all placeholder-slate-400 font-medium text-[14px] hover:bg-white leading-relaxed resize-none" placeholder="Tulis panduan langkah-langkah pembayaran untuk pelanggan..."><?= htmlspecialchars($method['instructions'] ?? '') ?></textarea>
                    ...
                    <div class="flex flex-col gap-4">
                        <div class="w-24 h-24 rounded-[16px] bg-slate-50 border-2 border-dashed border-slate-300 flex items-center justify-center text-slate-400 overflow-hidden shadow-sm relative group cursor-pointer transition-colors hover:border-indigo-400 hover:bg-indigo-50/50" onclick="document.getElementById('iconInput').click()">
                            <?php if(!empty($method['icon'])): ?>
                                <span id="previewPlaceholder" class="flex flex-col items-center gap-1 group-hover:text-indigo-500 transition-colors hidden">
                                    <i class="fas fa-credit-card text-[22px]"></i>
                                </span>
                                <div id="iconPreviewContainer" class="absolute inset-0 z-10">
                                    <img src="<?= BASEURL ?>/uploads/payment-methods/<?= htmlspecialchars($method['icon']) ?>" class="w-full h-full object-contain rounded-[16px] p-1">
                                </div>
                            <?php else: ?>
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-[12px] font-medium text-slate-400 mt-2.5 flex items-center gap-1.5"><i class="fas fa-circle-info text-slate-300"></i> Max 5MB. Format: JPG/PNG/WEBP. Kosongkan untuk mempertahankan ikon saat ini.</p>
                        </div>
                    </div>
                </div>
                <div>
                    <div class="mb-8">
                        <?php 
                            $props = [
                                ...
                                'type' => 'number',
                                'value' => htmlspecialchars($method['sort_order']),
                                'icon' => 'fas fa-sort'
                            ];
                            include __DIR__ . '/../components/ui/input.php';
                        ?>
                    ...
                    <div class="flex gap-4 p-1.5 bg-slate-100 rounded-xl w-fit border border-slate-200/60 p-1">
                        <label class="relative flex items-center justify-center cursor-pointer">
                            <input type="radio" name="is_active" value="1" <?= (!empty($method['is_active'])) ? 'checked' : '' ?> class="peer sr-only">
                        ...
                        <label class="relative flex items-center justify-center cursor-pointer">
                            <input type="radio" name="is_active" value="0" <?= (empty($method['is_active'])) ? 'checked' : '' ?> class="peer sr-only">
