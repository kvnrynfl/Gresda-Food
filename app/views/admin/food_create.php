...
<div class="mb-8 flex items-center justify-between">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-[26px] font-black text-slate-900 tracking-tight leading-none mb-1">Tambah Makanan Baru</h2>
            <p class="text-[13px] font-medium text-slate-500">Formulir penambahan inventaris menu publik</p>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-[24px] shadow-[0_2px_20px_-8px_rgba(0,0,0,0.1)] border border-slate-200/60 overflow-hidden w-full relative mb-8">
    ...
    <div class="p-8 sm:p-10">
        <form action="<?= BASEURL ?>/admin/createFood" method="POST" enctype="multipart/form-data" class="space-y-8">
            <?= CSRF::getTokenField() ?>
                ...
                <div>
                    <?php 
                        $props = [
                            ...
                            'placeholder' => 'misal: Burger Kentang Madu',
                            'required' => true
                        ];
            ...
            <div>
                ...
                <div class="relative w-full md:w-1/2">
                    ...
                    <input type="text" id="priceDisplay" required class="w-full pl-12 pr-5 py-3.5 bg-slate-50/50 border border-slate-200/80 rounded-xl text-slate-900 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all placeholder-slate-300 font-black text-[15px] font-mono tracking-tight hover:bg-white" placeholder="0">
                    <input type="hidden" name="price" id="priceValue" required>
                </div>
            </div>

            <div>
                <label class="block text-[13px] font-black text-slate-800 mb-2.5 uppercase tracking-wide">Naskah Penjualan (Deskripsi)</label>
                <textarea name="description" rows="4" required class="w-full px-5 py-3.5 bg-slate-50/50 border border-slate-200/80 rounded-xl text-slate-900 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all placeholder-slate-400 font-medium text-[14px] hover:bg-white leading-relaxed resize-none" placeholder="Tulis deskripsi persuasif tentang menu eksperimen Anda di sini..."></textarea>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
                <div>
                    <label class="block text-[13px] font-black text-slate-800 mb-2.5 uppercase tracking-wide">Representasi Visual (Opsional)</label>
                    <div class="flex flex-col gap-4">
                        <div class="w-24 h-24 rounded-[16px] bg-slate-50 border-2 border-dashed border-slate-300 flex items-center justify-center text-slate-400 overflow-hidden shadow-sm relative group cursor-pointer transition-colors hover:border-indigo-400 hover:bg-indigo-50/50" onclick="document.getElementById('imageInput').click()">
                            ...
                            <div id="imagePreviewContainer" class="absolute inset-0 z-10 hidden"></div>
                        </div>
                        <div class="flex-1">
                            ...
                            <p class="text-[12px] font-medium text-slate-400 mt-2.5 flex items-center gap-1.5"><i class="fas fa-circle-info text-slate-300"></i> Max 2MB. Format: JPG/PNG/WEBP.</p>
                        </div>
                    </div>
                </div>
                <div>
            ...
            <div class="pt-8 border-t border-slate-100 flex items-center justify-end gap-3 mt-4">
                <a href="<?= BASEURL ?>/admin/foods" class="px-6 py-3 rounded-xl text-[14px] font-extrabold text-slate-500 hover:text-slate-700 hover:bg-slate-100 transition-colors">Batal & Kembali</a>
                <button type="submit" class="px-8 py-3 rounded-xl text-[14px] font-black text-white bg-indigo-600 hover:bg-indigo-700 shadow-[0_4px_12px_rgba(79,70,229,0.3)] hover:shadow-[0_6px_16px_rgba(79,70,229,0.4)] hover:-translate-y-0.5 transition-all duration-300 flex items-center gap-2">
                    <i class="fas fa-check"></i> Simpan Publikasi
                </button>
            </div>
        </form>
    </div>
    ...
    document.getElementById('imageInput').addEventListener('change', function(e) {
        if(e.target.files && e.target.files[0]) {
            ...
            reader.onload = function(e) {
                ...
                container.innerHTML = `<img src="${e.target.result}" class="w-full h-full object-cover rounded-[16px]">`;
