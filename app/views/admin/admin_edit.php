...
<div class="mb-8 flex items-center justify-between">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-[26px] font-black text-slate-900 tracking-tight leading-none mb-1">Modifikasi Kredensial</h2>
            <p class="text-[13px] font-medium text-slate-500">Edit identitas kontrol akses untuk <?= htmlspecialchars($admin['username']) ?></p>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-[24px] shadow-[0_2px_20px_-8px_rgba(0,0,0,0.1)] border border-slate-200/60 overflow-hidden w-full relative mb-8">
    ...
    <div class="p-8 sm:p-10">
        <form action="<?= BASEURL ?>/admin/editAdmin/<?= $admin['id'] ?>" method="POST" class="space-y-8">
            <?= CSRF::getTokenField() ?>
            ...
            <div class="pt-8 border-t border-slate-100 flex items-center justify-end gap-3 mt-4">
                <a href="<?= BASEURL ?>/admin/admins" class="px-6 py-3 rounded-xl text-[14px] font-extrabold text-slate-500 hover:text-slate-700 hover:bg-slate-100 transition-colors">Dibatalkan</a>
                <button type="submit" class="px-8 py-3 rounded-xl text-[14px] font-black text-white bg-indigo-600 hover:bg-indigo-700 shadow-[0_4px_12px_rgba(79,70,229,0.3)] hover:shadow-[0_6px_16px_rgba(79,70,229,0.4)] hover:-translate-y-0.5 transition-all duration-300 flex items-center gap-2">
                    <i class="fas fa-check"></i> Simpan Mutasi
                </button>
            </div>
        </form>
    </div>
