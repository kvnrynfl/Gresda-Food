...
<div class="mb-8 flex items-center justify-between">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-[26px] font-black text-slate-900 tracking-tight leading-none mb-1">Tambah Kategori Baru</h2>
            <p class="text-[13px] font-medium text-slate-500">Definisi pengelompokkan untuk sistem katalog</p>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-[24px] shadow-[0_2px_20px_-8px_rgba(0,0,0,0.1)] border border-slate-200/60 overflow-hidden w-full relative mb-8">
    ...
    <div class="p-8 sm:p-10">
        <form action="<?= BASEURL ?>/admin/createCategory" method="POST" class="space-y-8">
            <?= CSRF::getTokenField() ?>
            ...
            <?php 
                $props = [
                    ...
                    'placeholder' => 'misal: Burger, Pizza, Minuman Spesial',
                    'required' => true
                ];
                include __DIR__ . '/../components/ui/input.php';
            ?>
            <p class="text-[12px] font-medium text-slate-400 -mt-3 mb-6 flex items-center gap-1.5"><i class="fas fa-circle-info text-slate-300"></i> Judul ini yang akan dibaca oleh pelanggan pangkalan.</p>

            <?php 
                $props = [
                    'name' => 'slug',
                    'label' => 'Kunci Registri Akses (Slug URL)',
                    'placeholder' => 'burger-super',
                    'icon' => 'fas fa-link',
            ...
            <div>
                ...
                <div class="flex gap-4 p-1.5 bg-slate-100 rounded-xl border border-slate-200/60 p-1">
                    <label class="relative flex-1 flex items-center justify-center cursor-pointer">
                        <input type="radio" name="active" value="Yes" checked class="peer sr-only">
                    ...
                    <label class="relative flex-1 flex items-center justify-center cursor-pointer">
                        <input type="radio" name="active" value="No" class="peer sr-only">
            ...
            <div class="pt-8 border-t border-slate-100 flex items-center justify-end gap-3 mt-4">
                ...
                <button type="submit" class="px-8 py-3 rounded-xl text-[14px] font-black text-white bg-indigo-600 hover:bg-indigo-700 shadow-[0_4px_12px_rgba(79,70,229,0.3)] hover:shadow-[0_6px_16px_rgba(79,70,229,0.4)] hover:-translate-y-0.5 transition-all duration-300 flex items-center gap-2">
                    <i class="fas fa-folder-plus"></i> Finalisasi Kategori
                </button>
            </div>
        </form>
    </div>
...
<script>
    // Auto-generate slug from category name with pristine transition UX
